...
This is a module for Foundry Virtual Tabletop that adds a "Quick Actions" menu above the Token HUD when right-clicking on tokens. This menu shows all Features, Spells, and Items for the actor that has an Activation Cost of Action, Bonus Action, Reaction, Legendary Action, Lair Action, Special, or Crew Action. If the action has limited uses, the menu indicates how many uses the actor has for that action. This menu can optionally be made available to other players. Features that are passive in nature and not intended to be triggered manually are excluded from this list. 
